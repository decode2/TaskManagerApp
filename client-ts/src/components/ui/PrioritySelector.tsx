  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.99 }}
      >
        <PriorityBadge priority={value} variant="minimal" animated={false} />
        <motion.div
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg overflow-hidden"
          >
            {priorities.map((priority, index) => (
              <motion.button
                ...
                className={`
                  ...
                  ${value === priority ? 'bg-blue-50 dark:bg-blue-900/30' : ''}
                `}
                ...
                whileHover={{ x: 4 }}
              >
                <PriorityBadge priority={priority} variant="minimal" animated={false} />
                {value === priority && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
